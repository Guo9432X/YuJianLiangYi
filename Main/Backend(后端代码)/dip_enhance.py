...
def full_dip_enhance(image, use_light_enhance=True, use_clahe=True, use_sharpen=True):
    """
    完整的 DIP 增强流水线
    - use_light_enhance: 是否执行自适应光照增强（周嘉明模块）
    - use_clahe: 是否执行 CLAHE + 锐化
    注意：若 use_light_enhance 和 use_clahe 同时启用，先做光照增强再做 CLAHE
    """
    img = image.copy()
    if use_light_enhance:
        img = adaptive_light_enhance(img, auto_threshold=100, strength=1.0)
    if use_clahe:
        img = enhance_disease_region(img)   # 包含 CLAHE 和锐化
    return img
